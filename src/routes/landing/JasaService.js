...
import JasaService from '../../data/JasaService';
...
export default class MenuPricingPreview extends React.Component {
  render() {
    return (
      ...
      >
        <h2 className="txt-center pop-font txt-white">Jasa Service</h2>
        <p className="section-description">
          Pilih jasa yang kamu butuhkan. Kami siap membantu!
        </p>
        <section className="pricing-grid flex-container flex-column">
          {JasaService.map((menu) => (
            <Tilt key={menu.id}>
              <Link
